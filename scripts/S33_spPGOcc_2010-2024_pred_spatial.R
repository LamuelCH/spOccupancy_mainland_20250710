#!/usr/bin/env Rscript
# ============================================================================
# SINGLE-SPECIES Spatial Prediction using spPGOcc Model
# ============================================================================
# Description: This script performs spatial predictions for singleple species
#              using a fitted spPGOcc model across a study area defined by
...
base_output_dir <- paste0("output/spPGOcc/predictions/spatial/pred_", current_date)
...
model_file <- "models/spPGOcc/model_20250710_spPGOcc_2010-2024_nthin1000_nburn1e+06_nchain1_nsample6000_neighbors10.RData"
...
if (!is.null(out.spPGOcc$y)) {
  n_species <- 1
  species_names <- c("Dasyurus")
  ...
  n_species <- 1
  species_names <- c("Dasyurus")
  cat("\nÃÂÃ  Could not determine species names from model\n")
...
for (i in 1:length(env_vars)) {
  ...
  if (n_nan > 0 || n_inf > 0) {
    cat("  ÃÂÃ ", env_vars[i], ": Found", n_nan, "NaN and", n_inf, "Inf values\n")
...
if (n_na > 0) {
  cat("  ÃÂÃ  Found", format(n_na, big.mark = ","), "rows with NA values\n")
  X.0 <- X.0[!na_rows, ]
...
for (i in 1:n_chunks) {
  ...
  tryCatch({
    ...
    # Get dimensions - for single species model
    n_loc <- dim(pred.chunk$psi.0.samples)[2]  # Should be [n.samples, n.locations]
    
    # Process each location
    for (loc in 1:n_loc) {
      # Extract spatial random effect for this location
      w_mean <- mean(pred.chunk$w.0.samples[, loc])
      w_sd <- sd(pred.chunk$w.0.samples[, loc])
      
      # Calculate summary statistics (no species loop needed for single species)
      psi_mean <- mean(pred.chunk$psi.0.samples[, loc])
      psi_sd <- sd(pred.chunk$psi.0.samples[, loc])
      z_prob <- mean(pred.chunk$z.0.samples[, loc])
      
      # Create row
      row_data <- data.frame(
        x = coords.0.chunk[loc, 1],
        y = coords.0.chunk[loc, 2],
        species = 1,  # Always 1 for single species
    ...
    if (grepl("cannot allocate", conditionMessage(e))) {
      cat("ÃÂÃ  Memory allocation error detected. Attempting smaller sub-chunks...\n")
...
if (nrow(results_summary) > 0) {
  ...
  # Convert coordinates back to meters for raster creation
  results_summary$x_m <- results_summary$x * 1000
  results_summary$y_m <- results_summary$y * 1000
  ...
  for (sp in species_ids) {
    ...
    psi_mean_raster <- rast(sp_data[, c("x_m", "y_m", "psi_mean")], type = "xyz", crs = "EPSG:3577")
    ...
    # Uncertainty
    psi_sd_raster <- rast(sp_data[, c("x_m", "y_m", "psi_sd")], type = "xyz", crs = "EPSG:3577")
    ...
    z_prob_raster <- rast(sp_data[, c("x_m", "y_m", "z_prob")], type = "xyz", crs = "EPSG:3577")
    ...
    w_mean_raster <- rast(sp_data[, c("x_m", "y_m", "w_mean")], type = "xyz", crs = "EPSG:3577")
    writeRaster(w_mean_raster, file.path(species_dir, "w_mean.tif"), overwrite = TRUE)
    cat("     w_mean.tif (mean spatial random effect)\n")
    
    w_sd_raster <- rast(sp_data[, c("x_m", "y_m", "w_sd")], type = "xyz", crs = "EPSG:3577")
...
cat("\n SINGLE-SPECIES spatial prediction completed successfully!\n")
cat("========================================\n\n")