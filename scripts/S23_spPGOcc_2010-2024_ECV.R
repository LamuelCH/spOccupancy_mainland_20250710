...
# Enhanced Version with Configurable K-Fold Environmental Clustering Cross-Validation
# =============================================================================
...
tryCatch({
  ...
  plot_file <- file.path(diagnostics_dir, paste0("spatial_blocks_", K_FOLDS, "fold_", 
  ...
  cat("\n=== STEP 9: SPATIAL CROSS-VALIDATION MODEL FITTING ===\n")
  ...
  # Fit the spatial model with cross-validation
  cat("\n--- SPATIAL", K_FOLDS, "FOLD ENVIRONMENTAL CLUSTERING CV MODEL FITTING IN PROGRESS ---\n")
  ...
  cat("\n--- SPATIAL", K_FOLDS, "FOLD ENVIRONMENTAL CLUSTERING CV MODEL FITTING COMPLETED ---\n")
  ...
  if(!is.null(model_result)) {
    ...
    cat("\n SPATIAL", K_FOLDS, "FOLD ENVIRONMENTAL CLUSTERING CV MODEL FITTING SUCCESSFUL!\n")
   
    
  } else {
    cat("Â  SPATIAL", K_FOLDS, "FOLD ENVIRONMENTAL CLUSTERING CV MODEL FITTING FAILED\n")
  ...
  writeLines(c(
    ...
    paste("Model Type: spPGOcc (Spatial Single-Species with", K_FOLDS, "fold", CV_TYPE, ")"),
    paste("Target Species: Dasyurus"),
...
if(exists("out.spPGOcc") && !is.null(out.spPGOcc)) {
  ...
  write.csv(fold_summary, 
            file.path(diagnostics_dir, paste0(K_FOLDS, "fold_summary_", 
